...
def get_predicted_value(patient_symptoms, model):
    try:
        # Create a zero vector with 132 features
        ...
        # Check if prediction exists in diseases list
        if prediction in diseases_list:
            return diseases_list[prediction]
        else:
            return "Unable to make a confident prediction. Please consult a healthcare provider."
            
    except Exception as e:
        return "Unable to process these symptoms. Please try a different combination or consult a healthcare provider."
    ...
    if st.sidebar.button("Get Diagnosis"):
        if len(selected_symptoms) > 0:
            with st.spinner("Analyzing symptoms..."):
                ...
                if "Unable to" in predicted_disease:
                    st.warning(predicted_disease)
                    st.info("Tips for better results:")
                    st.markdown("""
                    - Try selecting fewer symptoms
                    - Focus on your main symptoms
                    - Make sure symptoms are related
